input = """light gold bags contain 2 light lime bags, 1 faded green bag, 3 clear olive bags, 2 dim bronze bags.
...
input_split = input.replace(" bags.", "").replace("bags", "bag").split("\n")
bags_list = []
bags_tuples = []
for item in input_split:
    bags_list.append(item.split(" bag contain "))
bags_dicts = {}
for item in bags_list:
    outer_bag = item[0]
    inner_bags_list = item[1].split(" bag, ")
    inner_bags = {}
    for bag in inner_bags_list:
        if bag != "no other":
            inner_bags[bag[2:]] = int(bag[0])
        else:
            pass
    bags_dicts[outer_bag] = inner_bags
    bags_tuples.append((outer_bag, inner_bags))
